...
def default_ai_hub_root() -> Path:
    if is_windows():
        drive = os.environ.get("SystemDrive") or "C:"
        return Path(drive + "\\") / "AI Hub OS"
    return Path.home() / "AI Hub OS"


def default_install_path(ai_hub_root: Path | None = None) -> Path:
    return (ai_hub_root or default_ai_hub_root()) / "Hermes"


def default_obsidian_path(ai_hub_root: Path | None = None) -> Path:
    return (ai_hub_root or default_ai_hub_root()) / "Obsidian"


def default_llm_wiki_path(ai_hub_root: Path | None = None) -> Path:
    return (ai_hub_root or default_ai_hub_root()) / "LLM Wiki"


def default_hermes_base_home(ai_hub_root: Path | None = None) -> Path:
    return default_install_path(ai_hub_root) / ".hermes"


def resolve_ai_hub_root(path: Path, runner: StepRunner, explicit: bool = False) -> Path:
    if runner.dry_run:
        return path
    try:
        ...
        if is_windows() and not explicit:
            fallback_base = os.environ.get("LOCALAPPDATA") or str(Path.home() / "AppData" / "Local")
            fallback = Path(fallback_base) / "AI Hub OS"
            runner.warn(f"Cannot create {path}; falling back to {fallback}")
...
def download_file(url: str, dest: Path, runner: StepRunner) -> None:
    runner.info(f"download {url} -> {dest}")
    if runner.dry_run:
        return
    ...
    if curl:
        try:
            runner.run([curl, "-fsSL", url, "-o", str(dest)])
            return
        except subprocess.CalledProcessError:
            runner.warn("curl download failed; retrying with Python urllib.")
...
def write_file_if_absent(path: Path, text: str, runner: StepRunner) -> None:
    runner.info(f"write-if-absent {path}")
    if runner.dry_run:
        return
    if path.exists():
        runner.ok(f"Keeping existing file: {path}")
        return
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(text, encoding="utf-8")
...
def append_env_value(path: Path, key: str, value: Path, runner: StepRunner) -> None:
    line = f"{key}={value}"
...
- `raw/sources/`: source material. Read it, never modify it unless the user asks.
- `raw/assets/`: local images and attachments from sources.
- `wiki/`: LLM-generated pages, summaries, entity pages, topic pages, and outputs.
- `index.md`: content map. Update it after every ingest or major query.
- `log.md`: chronological record. Append every ingest, query, and lint pass.

## Workflow

1. Ingest one source at a time unless the user asks for batch work.
2. Create or update relevant wiki pages.
3. Add Obsidian links with `[[Page Name]]` when pages are related.
4. Note contradictions and stale claims explicitly.
5. Update `index.md` and append `log.md` before finishing.
6. Keep user secrets out of wiki files.
""",
        runner,
    )
...
def install_obsidian(runner: StepRunner, skip: bool = False) -> None:
    if skip:
        runner.warn("Skipping Obsidian install/check (--skip-obsidian).")
        return
    if is_macos():
        if Path("/Applications/Obsidian.app").exists() or Path.home().joinpath("Applications/Obsidian.app").exists():
            runner.ok("Obsidian already installed; skipping.")
            return
        install_homebrew_if_missing(runner)
        if command_exists("brew"):
            runner.run(["brew", "install", "--cask", "obsidian"], check=False)
            return
        runner.warn("Obsidian not installed and Homebrew unavailable; skipping app install.")
    elif is_windows():
        if command_exists("winget"):
            found = runner.run(["winget", "list", "--id", "Obsidian.Obsidian", "-e"], check=False)
            if found.returncode == 0:
                runner.ok("Obsidian already installed; skipping.")
                return
            runner.run(["winget", "install", "--id", "Obsidian.Obsidian", "-e", "--accept-source-agreements", "--accept-package-agreements"], check=False)
        else:
            runner.warn("winget unavailable; skipping Obsidian app install.")
    else:
        runner.info("Obsidian app auto-install is only handled on macOS/Windows; skipping.")
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--ai-hub-root", help="Base folder. Default: C:\\AI Hub OS on Windows, ~/AI Hub OS elsewhere.")
    p.add_argument("--install-path", help="Hermes install path. Default: <AI Hub OS>/Hermes")
...
def main() -> int:
    ...
    ai_hub_root = resolve_ai_hub_root(ai_hub_root, runner, explicit=ai_hub_root_explicit)
    default_path = str(default_install_path(ai_hub_root))
    ...
    hermes_base = Path(os.environ.get("HERMES_BASE_HOME") or default_hermes_base_home(ai_hub_root)).expanduser().resolve()
    ...
    print(f"LLM Wiki   : {llm_wiki_path}")
